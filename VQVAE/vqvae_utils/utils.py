...
warnings.filterwarnings('ignore')                       # to avoid raising all warnings from eulertoexpmap
...
def load_all_data(wav_name: str, speaker_motion: bool) -> Tuple[np.array, pd.DataFrame, np.array]:
    ...
    pipeline = create_TAG2G_pipeline()
    bvh_data = process_pipeline(load_bvh_file(bvh_name), pipeline)
