...
class MocapParameterizer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, param_type = 'euler', bworld_translation_scaler=100,  ):
        '''
        
        param_type = {'euler', 'quat', 'expmap', 'position', 'expmap2pos'}
        '''
        self.param_type = param_type
        self.bworld_translation_scaler = bworld_translation_scaler
...
#        return X
    ...
    def inverse_transform(self, X, copy=None): 
        ...
        elif self.param_type == 'TAG2G_expmap':
            return self.TAG2G_expmap_to_euler(X)
        elif self.param_type == 'quat':
    ...
    def _to_expmap(self, X):
        ...
        for track in X:
            ...
            # print(f"From mocapParametrizer columns new_track.values: {len(list(new_track.values.columns))}") # debug line
...
class TAG2G_MocapParameterizer(BaseEstimator, TransformerMixin):

    def __init__(self, param_type='TAG2G_expmap', bworld_translation_scaler=100):
    ...
    def TAG2G_to_expmap(self, X):

        '''Converts Euler angles+ body_world translation to Exponential Maps'''
        ...
        for track in X:
            ...
            # == From here euler rotations to expmap rotations  ==
            ...
            # normalize rotation and movement - when interloctr's data
            norm_trasl = [joint for joint in track.values.columns if track.root_name in joint and 'position' in joint]
            ...
            if euler_df[norm_rot[2]].abs().mean() > 150:        # this means the figure is 180degrees rotated
                ...
                # traslation directly into exp_df (otherwise change in coordinate won't happen from euler_df)
                exp_df[norm_trasl[0]] = - exp_df[norm_trasl[0]]
                exp_df[norm_trasl[2]] = - exp_df[norm_trasl[2]]
            ...
            # == From here normalize and scale body_world positions ==

            body_world = X[0].root_name
            joints = (joint for joint in track.values.columns if (body_world in joint) and ('rotation' not in joint))

            for joint in joints:
                # using mean to avoid pure parametric translation
                exp_df[joint] = (exp_df[joint] - exp_df[joint].mean()) / self.bworld_translation_scaler

            # define the new track taking from exp_df with same skeleton data
            new_track = track.clone()
            new_track.values = exp_df
    ...
    def TAG2G_expmap_to_euler(self, X):

        """ Convert expmap + body_world translation to euler """
        ...
        for track in X:
            ...
            # == From here TAG2G_expmap to euler rotations ==
            ...
            # == From here body_world parametrized translation to euler rotations ==

            body_world = X[0].root_name
            joints = (joint for joint in track.values.columns if (body_world in joint) and ('position' in joint))

            for joint in joints:
                # print(f"DEBUG: scaling back {joint} with {self.bworld_translation_scaler}")
                euler_df[joint] = euler_df[joint] * self.bworld_translation_scaler
...
class JointSelector(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            # debug stuff
            # print(f"track values shape: {track.values.shape}")
            t2.values = track.values[self.selected_channels]
            # print(f"t2 values shape: {t2.values.shape}")
            # print(f"These are the selected channels: \n{self.selected_channels}")
...
class TAG2G_JointSelector(BaseEstimator, TransformerMixin):
    '''
    Allows for filtering the mocap data to include only the selected joints
    '''

    def __init__(self, joints, include_body_world=False):
        self.joints = joints
        self.include_body_world = include_body_world

    def fit(self, X, y=None):
        selected_joints = []
        selected_channels = []

        if self.include_body_world:
            selected_joints.append(X[0].root_name.replace("body_",""))

        selected_joints.extend(self.joints)

        # print("Inside for loop -jsel")                            # DEBUG LINE
        for joint_name in selected_joints:
            # print(f"{joint_name}")                                # DEBUG LINE
            selected_channels.extend(
                [o for o in X[0].values.columns if ('_' + joint_name + '_') in o and 'Nub' not in o])

        self.selected_joints = selected_joints
        self.selected_channels = selected_channels
        self.not_selected = X[0].values.columns.difference(selected_channels)
        # print(f"Not selected joints: \n{self.not_selected}")        # DEBUG LINE
        self.not_selected_values = {c: X[0].values[c].values[0] for c in self.not_selected}
...
class ConstantsRemover(BaseEstimator, TransformerMixin):
    ...
    It removes from the track.values (pd.DataFrame) all the entry (ie. columns) that have static values:
        this means unused joints or impractical joints from data (ie. knee can't rotate seen from frontal side)
        or other elements like b_world_rotations ... and so on
