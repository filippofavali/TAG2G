...
from system import VQVAESystem, VQVAEDataModule
import shutil
import random
import logging
...
class SystemSelector:
    ...
    def add_system_args(parent_parser: ArgumentParser):
        ...
        parser.add_argument('--input_dim', type=int, default=74)
    ...
    def initialize(self):
        ...
        self.initialize_system(num_embeddings, embedding_dim, input_dim, hidden_dim, gamma)

    def initialize_system(self, num_embeddings: int, embedding_dim: int, input_dim: int,
                          hidden_dim: int, gamma : float, max_frames: int = MAX_BEATS_LEN):
        self.system = VQVAESystem(num_embeddings, embedding_dim, input_dim, hidden_dim, max_frames, gamma=gamma)
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument("--cuda_n", type=int, default=0)
    ...
    # trainer_device = f"cuda:{args.cuda_n}" if torch.cuda.is_available() else "cpu"      # DOESN'T WORK
