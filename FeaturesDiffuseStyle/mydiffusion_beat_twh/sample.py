        ...
        try:
            ...
            for speaker in args.speakers:
                ...
                if speaker == 'main-agent':
                    model_kwargs_[label]['audio'] = textaudio[args.n_seed:, i:i+1, ...].transpose(0, 1)
                    if i == 0:
                ...
                if speaker == 'interloctr':
                    if i == 0:
                        ...
                        # Load gesture from sample and
                        model_kwargs_[label]['audio'] = textaudio[args.n_seed:, i-1:i, ...].transpose(0, 1)
                        gesture = data[speaker]['gesture']
                        # TODO: Check that gesture has the requirred len at last subdivision otherwise pad zero
                        gesture = (gesture - gesture_stats[speaker]['mean']) / gesture_stats[speaker]['std']
                        # gesture from i*n_poses-2 to i*n_poses+n_poses  (-2 to compute vel and acc)
                        gesture = gesture[i*args.n_poses-2:i*args.n_poses+args.n_poses, ...]   # 10, 256
    ...
    if tst_path is not None:
        ...
        for i, filename in progress_bar:
            ...
            try:
                ...
                print(f"'{filename}' not found in tst path provided")
...
if __name__ == '__main__':
    ...
    """
    --Hypotesys:
    a. there are some models that can generate only in the monadic versionù
