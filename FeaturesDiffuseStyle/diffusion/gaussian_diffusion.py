...
from FeaturesDiffuseStyle.diffusion.nn import mean_flat, sum_flat
from FeaturesDiffuseStyle.diffusion.losses import normal_kl, discretized_gaussian_log_likelihood
    ...
    def __init__(
        ...
        *,
        ...
        if vqvae is not None:
            self.use_vqvae_decoder = True
            self.vqvae = vqvae
        else:
            self.use_vqvae_decoder = False
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:     # LossType.MSE
            ...
            # see if any model_output arrangement is needed
                ...
                model_output = self.vqvae.vqvae.decoder(model_output).unsqueeze(2)
                target = model_kwargs['y']['motion_gt']
            # print(f"This is target shape: {target.shape}")
            else:
                target = {
                    ...
                    # x_start is passed from forward_backward in train_loop.run_step -> x_start = micro = batch

            assert model_output.shape == target.shape  # [bs, njoints, nfeats, nframes]
